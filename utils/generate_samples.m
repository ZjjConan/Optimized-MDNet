function [ bb_samples ] = generate_samples(type, bb, n, opts, trans_f, scale_f)
% GEN_SAMPLES
...
h = opts.imageSize(1); w = opts.imageSize(2);
...
switch (type)
    case 'gaussian'
        ...
        samples(:,3:4) = samples(:,3:4) .* repmat(opts.scaleFactor.^(scale_f*max(-1,min(1,0.5*randn(n,1)))),1,2);
    case 'uniform'
        samples(:,1:2) = samples(:,1:2) + trans_f * round(mean(bb(3:4))) * (rand(n,2)*2-1);
        samples(:,3:4) = samples(:,3:4) .* repmat(opts.scaleFactor.^(scale_f*(rand(n,1)*2-1)),1,2);
    case 'uniform_aspect'
        samples(:,1:2) = samples(:,1:2) + trans_f * repmat(bb(3:4),n,1) .* (rand(n,2)*2-1);
        samples(:,3:4) = samples(:,3:4) .* opts.scaleFactor.^(rand(n,2)*4-2);
        samples(:,3:4) = samples(:,3:4) .* repmat(opts.scaleFactor.^(scale_f*rand(n,1)),1,2);
    case 'whole'
        ...
        windows = [dx(:) dy(:) bb(3)*opts.scaleFactor.^ds(:) bb(4)*opts.scaleFactor.^ds(:)];
