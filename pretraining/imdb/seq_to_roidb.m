function [ roidb ] = seq_to_roidb(config, opts)
% copy from MDNet
%   https://github.com/HyeonseobNam/MDNet/blob/master/pretraining/seq2roidb.m
%
...
for i=1:length(images)
    ...
    while(size(pos_examples,1)<opts.posPerFrame-1)
        pos = genSamples(targetLoc, opts.posPerFrame*5,...
            imgSize, opts.scaleFactor, 0.1, 5, false);
        r = bboxOverlapRatio(pos,targetLoc);
    ...
    while(size(neg_examples,1)<opts.negPerFrame)
        neg = genSamples(targetLoc, opts.negPerFrame*2,...
            imgSize, opts.scaleFactor, 2, 10, true);
        r = bboxOverlapRatio(neg,targetLoc);
    ...
    rois(i).pos_boxes = [i * ones(size(pos_examples, 1)+1, 1), single([targetLoc; pos_examples])];
    rois(i).neg_boxes = [i * ones(size(neg_examples, 1), 1), single(neg_examples)];
end
