function [net,stats] = cnn_train_net(net, imdb, getBatch, varargin)
...
% ------------------------
% opts.resetImdbPerEpoch = [];
% opts.resetImageSize = [];
% opts.maxSamplingStep = [];
...
%   if ~isempty(opts.resetImdbPerEpoch)
%     imdb_ = imdb;
%     imdb = setupImdb(imdb_, opts);
%     opts.train = 1:imdb.images.offset;
...
if strcmpi(mode, 'train')
    epochSize = max(params.epochSize, numel(subset));
else
    epochSize = numel(subset);
end
% -------------------------------------------------
adjustTime = 0 ;
...
% net.vars(net.getVarIndex('k')).precious = 1;

start = tic ;
for t=1:params.batchSize:epochSize
  fprintf('%s: epoch %02d: %3d/%3d:', mode, epoch, ...
          fix((t-1)/params.batchSize)+1, ceil(epochSize/params.batchSize)) ;
  t = mod(t, numel(subset));
  if t == 0, t = 1; end
...
fprintf('------------------------------------------------------------\n');
fprintf('%s: epoch %02d: total video %4d:', mode, epoch, numel(subset));

% Save back to state.
state.stats.(mode) = stats ;
for f = setdiff(fieldnames(stats)', {'num', 'time'})
    f = char(f) ;
    valid = state.stats.(mode).(f) ~= 0;
    state.stats.(mode).(f) = mean(state.stats.(mode).(f)(valid));
    fprintf(' mean %s: %.3f', f, state.stats.(mode).(f));
end
fprintf('\n');
fprintf('------------------------------------------------------------\n');
